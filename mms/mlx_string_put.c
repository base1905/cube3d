...
int mlx_put_image_to_window_MAP(void *mlx_ptr, void *win_ptr, void *img_ptr, int sx, int sy, int sw, int sh, int dx, int dy, int dw, int dh, unsigned int color);
...
int mlx_string_put(void *mlx_ptr, void *win_ptr, int x, int y, int color, char *string)
{
  ...
  while (*string)
	...
	val = 31;
      mlx_put_image_to_window_MAP(mlx_ptr, win_ptr, font, val*(FONT_WIDTH+2), 0, FONT_WIDTH, font_atlas.height, x+pos*dest_w, y, dest_w, dest_h, color);
      pos ++;
      string ++;
    }
  return (0);
